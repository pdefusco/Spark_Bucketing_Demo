...
* The max number of possible files created is 625 for perfectly evenly distributed data, and would be less depending on how much skew there is. In my case, my 100M rows are perfectly distributed because I have a monotonically increasing "unique_id" so I get 625 files.
...
* In this case this is "code"  which uniformly takes a value among a list of 12 string codes. But I know this because I generate the synthetic data. In your case you'd have to do some tests with your data.
This creates a maximum of 12 partitions x 25 buckets, reflecting the number of unique code categories and requested buckets.
...
spark.sql("""SELECT CODE, COUNT(*)
            FROM {0}.{1}
            GROUP BY CODE""".format(src_tbl, tgt_tbl))
```
...
* However, this is hard to achieve because the data may not lend itself to this. Regardless, testing different combinations of repartitioning and bucketing keys with the objective of getting to an as small as possible number of files with roughly 128 MB of data each is an exercise that goes in the direction of this "ideal" scenario.  
* In the below example, the data is repartitioned by the low cardinality and evenly distributed "code" column, as before, but this time the number of partitions is reduced to 5 resulting in 125 files i.e. a lower number of files each with more data than the previous example 3.
